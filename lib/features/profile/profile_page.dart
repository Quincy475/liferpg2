      ...
      body: AtmosphereBackground(
        child: SafeArea(
          child: authAsync.when(
            ...
            data: (authUser) {
              if (authUser == null) return const _AuthWelcomeView();
              return _ProfileContent(authUser: authUser);
            },
          ),
        ),
      ),
...
class _AuthWelcomeView extends ConsumerStatefulWidget {
  const _AuthWelcomeView();

  @override
  ConsumerState<_AuthWelcomeView> createState() => _AuthWelcomeViewState();
}

class _AuthWelcomeViewState extends ConsumerState<_AuthWelcomeView> {
  ...
  Future<void> _createDevAccount() async {
    ...
    try {
      final cred = await ref.read(authRepoProvider).signInAnonymously();
      await ref.read(fsUserRepoProvider).ensureUserDoc(cred.user!.uid, defaultName: 'Guest');
  ...
  Future<void> _registerWithEmail() async {
    ...
    if (email.isEmpty || password.length < 6) {
      setState(() => _message = 'Gebruik een geldig e-mailadres en minimaal 6 tekens wachtwoord.');
      return;
    }
    setState(() {
      _busy = true;
    ...
    try {
      final cred = await ref.read(authRepoProvider).createUserWithEmailAndPassword(
            email: email,
            password: password,
          );
      await ref.read(fsUserRepoProvider).ensureUserDoc(cred.user!.uid, defaultName: email.split('@').first);
    } on FirebaseAuthException catch (e) {
      setState(() => _message = e.message ?? e.code);
    } finally {
      if (mounted) setState(() => _busy = false);
  ...
  Future<void> _loginWithEmail() async {
    ...
    if (email.isEmpty || password.isEmpty) {
      setState(() => _message = 'Vul e-mail en wachtwoord in.');
      return;
    }
    setState(() {
      _busy = true;
    ...
    try {
      final cred = await ref.read(authRepoProvider).signInWithEmailAndPassword(
            email: email,
            password: password,
          );
      await ref.read(fsUserRepoProvider).ensureUserDoc(cred.user!.uid, defaultName: email.split('@').first);
            ...
            child: Padding(
              ...
              child: Column(
                ...
                children: [
                  ...
                  Text(
                    'Je bent nog niet ingelogd. Tijdens development kun je met 1 klik een account starten.',
                  ...
                  SizedBox(
                    ...
                    child: FilledButton.icon(
                      onPressed: _busy ? null : _createDevAccount,
                      icon: const Icon(Icons.rocket_launch),
                      label: const Text('Create dev account (1 click)'),
                    ),
                  ),
                  const SizedBox(height: 8),
                  Text(
...
class _ProfileContent extends ConsumerWidget {
  final User authUser;
  const _ProfileContent({required this.authUser});
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    return meAsync.when(
      ...
      data: (me) {
        if (me == null) {
          return const Center(child: Text('Profiel wordt aangemaakt...'));
        }

        return ListView(
          padding: const EdgeInsets.all(16),
          children: [
            EnterMotion(delayMs: 20, child: _AccountCard(authUser: authUser, me: me)),
            const SizedBox(height: 12),
            EnterMotion(delayMs: 40, child: _NameCard(me: me)),
            const SizedBox(height: 12),
            EnterMotion(delayMs: 80, child: _GuildCard(me: me)),
            const SizedBox(height: 12),
...
class _AccountCard extends ConsumerStatefulWidget {
  ...
  const _AccountCard({required this.authUser, required this.me});
...
class _AccountCardState extends ConsumerState<_AccountCard> {
  ...
  Future<void> _upgradeAnonymous() async {
    ...
    try {
      await ref.read(authRepoProvider).linkAnonymousWithEmailPassword(email: email, password: password);
      await ref.read(fsUserRepoProvider).updateName(widget.me.id, email.split('@').first);
      setState(() => _message = 'Account gekoppeld. Je kunt nu met e-mail inloggen.');
    } on FirebaseAuthException catch (e) {
      setState(() => _message = e.message ?? e.code);
    } finally {
      if (mounted) setState(() => _busy = false);
  ...
  @override
  Widget build(BuildContext context) {
    final isAnonymous = widget.authUser.isAnonymous;
    return Card(
      child: Padding(
          ...
          children: [
            Row(
              children: [
                ...
                Chip(label: Text(isAnonymous ? 'Guest' : 'Email account')),
                const Spacer(),
                TextButton.icon(
                  onPressed: _busy
                      ? null
            ...
            if (widget.authUser.email != null)
              Padding(
                padding: const EdgeInsets.only(bottom: 8),
                child: Text('Ingelogd als ${widget.authUser.email}'),
              ),
            if (isAnonymous) ...[
              const SizedBox(height: 8),
              Text('Maak je guest account permanent met e-mail.', style: Theme.of(context).textTheme.bodyMedium),
              const SizedBox(height: 10),
              TextField(
                controller: _emailC,
                keyboardType: TextInputType.emailAddress,
              ...
              const SizedBox(height: 10),
              FilledButton(
                onPressed: _busy ? null : _upgradeAnonymous,
